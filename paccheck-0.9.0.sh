...
#
# Compare pacman sync databases and package desc files, checking from different mirrors
# Verify size of packages in cache
# Compare packages downloaded from several mirrors
#
# Copy desired mirrors from /etc/pacman.d/mirrorlist
# and save them to /etc/paccheck/mirrorlist
#
# For best results use tier 1 mirrors
...
Full System Update Procedure:
    1) Run paccheck as user not root
...
while [[ "${mir[$x]}" != "" ]]; do
    ...
    if (( diag != 1 )); then
        ...
        if [[ "$?" == "0" ]]; then
            ...
            if (( verbose == 1 )); then
                echo "wget --no-verbose --tries=1 --connect-timeout=30 -O mirror-info.html ${infourl[@]}"
            fi
            test=$(wget --no-verbose --tries=1 --connect-timeout=30 -O mirror-info.html "${infourl[@]}" 2>&1)
...
for m in ${mir[@]}; do
    ...
    if (( checkpkg == 1 )); then
        ...
        if (( allok == 1 )); then
	    if (( ccnt == 1 )); then
		echo "      OK - The package matches in database"
	    else
...
if (( skipsize != 1 )); then
    ...
    if (( allok == 1 )); then
	if (( scnt == 1 )); then
	    echo "    OK - The package is the correct size"
	else
...
if [[ "$sizepkgs" != "" ]]; then
    ...
    echo "indicates they are corrupt or have been modified"
    echo "or the database has been tampered with:"
    echo -e "$sizepkgs"
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo
    error3=1

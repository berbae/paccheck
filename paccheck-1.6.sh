...
if ((verify == 1)); then
	...
	if [[ -s sig/${fpkg}.sig ]];then
	    ...
	    if ((err == 1)); then
                ...
                echo "    VERIFY ERROR $err from gpg"
                errors=1
            fi
	else
	    echo "not signed yet!"
...
if [[ -n "$badpkgs" ]]; then
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo "LIST OF PACKAGE(S) WITH DESC FILE MISMATCH:"
    echo -e "$badpkgs"
    echo "This indicates compromised mirror(s)"
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo
    error3=1
fi
if [[ -n "$sizepkgs" ]]; then
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo "LIST OF PACKAGE(S) IN PACMAN CACHE WITH WRONG SIZE:"
    echo -e "$sizepkgs"
    echo "This indicates corruption or modification"
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo
    error3=1
fi
if [[ -n "$badsig" ]]; then
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo "LIST OF PACKAGE(S) IN PACMAN CACHE WITH BAD SIGNATURE:"
    echo -e "$badsig"
    echo "This indicates corruption or modification"
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo
...
if [[ -n "$badcmp" ]]; then
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo "LIST OF PACKAGE(S) WITH FULL COMPARE MISMATCH:"
    echo -e "$badcmp"
    echo "This indicates compromised mirror(s)"
    echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    echo
...
if ((errors == 1)); then
    echo "ERROR(S) DURING EXECUTION - This indicates malfunction(s)."
    echo
    error1=1
fi
if ((baddownload == 1)); then
    echo "DOWNLOAD FAILURE(S) - This indicates unresponsive mirror(s)"
    echo "or file(s) missing on the mirror(s)."
    echo
    error1=1
fi
if [[ -n "$misstargets" ]]; then
    echo "LIST OF TARGET(S) NOT FOUND IN PACMAN CACHE:"
    echo -e "$misstargets"
    echo "Not tested"
    echo
    error1=1
...
if [[ -n "$misspkgs" ]]; then
    echo "LIST OF PACKAGE(S) MISSING FROM THE INDICATED MIRROR:"
    echo -e "$misspkgs"
    echo "Not tested if missing from all mirrors above"
    echo
    error2=1
fi
if ((contents == 1)) && [[ -z "$badpkgs" ]]; then
    echo "DATABASE CONTENT MISMATCH - This indicates out of sync mirror(s)"
    echo "See http://www.archlinux.org/mirrors/status/"
    echo
    error1=1
fi
...
if [[ -z ${mir[1]} ]]; then
    ...
    echo
    error1=1
elif ((error1 + error2 + error3 + errors + baddownload + contents == 0)); then
    echo "All OK."
    echo
fi
